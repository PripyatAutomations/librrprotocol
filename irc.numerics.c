...
//      irc_name(cptr), mp->argv[2]);
   ...
   if (net_aj && *net_aj) {
      ...
      free(aj);
//   } else {
//      ui_print(tui_active_window(), "net_aj: key %s returned %s", key, net_aj);
   }
   free( (char *)net_aj );
...
//   ui_print("status",
//      "%s {bright-cyan}>>>{reset} Attached to rig {bright-cyan}%s.%s{reset} via {bright-magenta}IRC{reset} transport [{green}%s{reset}] {bright-cyan}<<<{reset}",
...
//   ui_print("status", "%s [{green}%s{reset}] 251 %s", get_chat_ts(0),
//      irc_name(cptr), buf);
...
//   ui_print(tui_active_window(), "%s [{green}%s{reset}] * %s!%s@%s <%s>", get_chat_ts(0),
//      irc_name(cptr), nick, ident, host, realname);
...
//   ui_print(tui_active_window(), "%s [{green}%s{reset}] * %s is on server %s: %s",
...
//   ui_print( tui_active_window(), "%s [{green}%s{reset}] *** 313 %s ***", get_chat_ts(0), buf,
//      irc_name(cptr) );
...
bool irc_builtin_num317(rrconn_t *cptr, irc_message_t *mp) {
   ...
   Log(LOG_DEBUG, "irc", "[%s] whois: %s connected at %s (idle %s)", irc_name(cptr), nick,
      signon_date, idle_ts);
//   ui_print(tui_active_window(),
//      "%s [{green}%s{reset}] * {bright-cyan}%s{reset} connected at {green}%s{reset} (idle {green}%s{reset})",
//      get_chat_ts(0), irc_name(cptr), nick, signon_date, idle_ts);
   free(idle_ts);
...
//   ui_print(tui_active_window(),
//      "%s [{green}%s{reset}] *** End of WHOIS {bright-cyan}%s{reset} ***", get_chat_ts(0),
//      irc_name(cptr), mp->argv[1]);
...
//   ui_print(tui_window_find(mp->argv[3]), "%s [{green}%s{reset}] *** %s ***", get_chat_ts(0),
//      irc_name(cptr), buf);
...
//   ui_print(tui_window_find(mp->argv[2]),
...
//   ui_print(tui_active_window(),
//      "%s [{green}%s{reset}] No such nickname: {bright-cyan}%s{reset}.", get_chat_ts(0),
//      irc_name(cptr), mp->argv[2]);
...
//   ui_print(tui_active_window(),
//      "%s [{green}%s{reset}] No such channel: {bright-magenta}%s{reset}.", get_chat_ts(0),
//      irc_name(cptr), mp->argv[2]);
...
//   ui_print("status", "%s [{green}%s{reset}] * %s Unknown Command => %s",
//      get_chat_ts(0), irc_name(cptr), nick, buf);
...
//   ui_print("status",
//      "%s [{green}%s{reset}] *** Nickname already in use: {bright-cyan}%s{reset}  ***",
//      get_chat_ts(0), irc_name(cptr), mp->argv[2]);
...
//   ui_print(tui_active_window(),
//      "%s [{green}%s{reset}] Not enough parameters: {bright-magenta}%s{reset} ", get_chat_ts(0),
//      irc_name(cptr), mp->argv[2]);
...
//   ui_print(tui_window_find(target),
...
const irc_numeric_t irc_numerics[] = {
   // --- Connection / welcome ---
   {
      ...
      .cb = irc_builtin_num001
   },
   {
      .code = 2, .name = "RPL_YOURHOST", .desc = "Your host information",
      .cb = irc_builtin_num_print
   },
   {
      .code = 3, .name = "RPL_CREATED", .desc = "Server creation time",
      .cb = irc_builtin_num_print
   },
   {
      .code = 4, .name = "RPL_MYINFO", .desc = "Server info and supported modes",
      .cb = irc_builtin_num004
   },
   {
      .code = 5, .name = "RPL_ISUPPORT", .desc = "Supported features (IRCv3 tokens)",
      .cb = irc_builtin_num005
   },
   {
      .code = 251, .name = "RPL_LUSERCLIENT", .desc = "Online local users",
      .cb = irc_builtin_num_print
   },
   {
      .code = 254, .name = "RPL_LUSERCHANNELS", .desc = "Channels on server",
      .cb = irc_builtin_num_print
   },
   {
   ...
   {
      .code = 312, .name = "RPL_WHOISSERVER", .desc = "WHOIS: server info",
      .cb = irc_builtin_num312
   },
   {
      .code = 313, .name = "RPL_WHOISOPERATOR", .desc = "WHOIS: operator status",
      .cb = irc_builtin_num313
   },
   {
      .code = 317, .name = "RPL_WHOISIDLE", .desc = "WHOIS: idle time", .cb = irc_builtin_num317
   },
   {
      .code = 318, .name = "RPL_ENDOFWHOIS", .desc = "End of WHOIS reply",
      .cb = irc_builtin_num318
   },
   {
      .code = 319, .name = "RPL_WHOISCHANNELS", .desc = "WHOIS: channels",
      .cb = irc_builtin_num319
   },
   {
   ...
   // --- Errors ---
   {
      .code = 401, .name = "ERR_NOSUCHNICK", .desc = "No such nick/channel",
      .cb = irc_builtin_num401
   },
   {
      .code = 402, .name = "ERR_NOSUCHSERVER", .desc = "No such server", .cb = NULL
   },
   {
      .code = 403, .name = "ERR_NOSUCHCHANNEL", .desc = "No such channel",
      .cb = irc_builtin_num403
   },
   {
   ...
   {
      .code = 421, .name = "ERR_UNKNOWNCOMMAND", .desc = "Unknown command",
      .cb = irc_builtin_num421
   },
   {
      .code = 432, .name = "ERR_ERRONEUSNICKNAME", .desc = "Erroneous nickname", .cb = NULL
   },
   {
      .code = 433, .name = "ERR_NICKNAMEINUSE", .desc = "Nickname already in use",
      .cb = irc_builtin_num433
   },
   {
   ...
   {
      .code = 461, .name = "ERR_NEEDMOREPARAMS", .desc = "Not enough parameters",
      .cb = irc_builtin_num461
   },
   {
   ...
   {
      .code = 465, .name = "ERR_YOUREBANNEDCREEP", .desc = "You are banned from this server",
      .cb = NULL
   },
   {
   ...
   {
      .code = 482, .name = "ERR_CHANOPRIVSNEEDED", .desc = "Channel operator privileges needed",
      .cb = irc_builtin_num482
   },
   {
   ...
   {
      .code = 903, .name = "RPL_SASLSUCCESS", .desc = "SASL auth success (deprecated alias)",
      .cb = NULL
   },
   {
   ...
   {
      .code = 0, .name = NULL, .desc = NULL, .cb = NULL
   }                                                                                                               // terminator
};
